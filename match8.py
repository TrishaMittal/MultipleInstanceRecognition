...
output = open('output_full_tea_nms', 'w')
...
def get_template_descriptors():
	fp1 = open('files/p_tea','r')
	names = fp1.read().split()
	desc = []
...
for sh in shelf.readlines():
	...
	for keys in predict[-1:-6:-1]:
		...
		while(count):
			...
			if found is not None:
				...
				#rsz = cv2.GaussianBlur(template,(3,3),0)
				#cv2.imwrite("op1.png", g);
				#cv2.imwrite("op2.png", template);
				flag = find_obj(template, g)
				if flag:
					cv2.rectangle(image, (startX, startY), (endX, endY), COLORS[c_count], 30)
					name = tpl_names[keys[0]].split('/')
					name = name[-1].split('.')
					check = True
					if(len(rects)!=0):
						for k in range(len(rects)):
							if(rects[k][-1] and findOverlap(rects[k][1:5], [startX, endX, startY, endY])>0.5 and rects[k][-2] >  best_match):
								check = False
								rects[k][-1] = False
								rects.append([name[0], startX, endX, startY, endY, maxVal, True])
								best_match = maxVal
					if(check):
						rects.append([name[0], startX, endX, startY, endY, maxVal, True])
						best_match = maxVal
						
					#output.write(name[0]+' '+str(startX)+' '+str(endX)+' '+str(startY)+' '+str(endY)+'\n')
					count = MAX_MIS_PREDICTIONS	
					c+=1
	...
	for r in rects:
		if r[-1]:
			output.write(r[0]+' '+str(r[1])+' '+str(r[2])+' '+str(r[3])+' '+str(r[4])+'\n')
