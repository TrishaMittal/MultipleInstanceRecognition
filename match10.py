		...
		while(count):
			...
			if found is not None:
				...
				if flag:
					...
					print 'r = ', 1.0/r
					c+=1
					curr_tpl = tpl_names[keys[0]]
					matchesV = []
					matchesL = []
					for num in brands[curr_tpl]:
						print num
						im = cv2.imread('products/tea/'+str(num)+'.jpg')
						temp_v = []
						temp_l = []
						for i in range(3):
							tpl_match = cv2.matchTemplate(roi[:, :, i], im[:,:,i], cv2.TM_CCOEFF_NORMED)	
					...
					for i in range(len(matchesV)):
						if(sum(matchesV[i])>best_val):
							...
							best_loc = matchesL[i]
							print 'match =', matchesV[i]
					best_loc = best_loc[0]
					#best_loc = (best_loc[0]+sX, best_loc[1]+sY)
					print best_loc
					t =  brands[curr_tpl]
					im = cv2.imread('products/tea/'+str(t[0])+'.jpg')
					lower_scale = max(0, ind - 3);
					upper_scale = min(len(scales), ind + 3)
					cv2.rectangle(image, (startX, startY), (endX, endY), COLORS[-2], 30)
					...
					cv2.rectangle(roi, (best_loc[0], best_loc[1]), (best_loc[0]+im.shape[1], best_loc[1]+im.shape[0]), COLORS[-1], 30)
					...
					count = MAX_MIS_PREDICTIONS
					cv2.imwrite('Auto_Results/ress.png',image)	
