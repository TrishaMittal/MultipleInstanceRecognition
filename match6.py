		...
		while(count):
			...
			if found is not None:
				...
				if flag:
					...
					output.write(tpl_names[keys[0]]+' '+str(startX/(1.0*image.shape[1]))+' '+str(endX/(1.0*image.shape[1]))+' '+str(startY/(1.0*image.shape[0]))+' '+str(endY/(1.0*image.shape[0]))+'\n')
					count = MAX_MIS_PREDICTIONS	
					c+=1
